...
use crate::web::{ctx, form, renderer::Renderer, PageError, PASSWORD_COKKIE};
...
pub async fn get_clip(
    ...
    // Create an ineer function for this renders so we do not have to repeat the
    // code over again
...
pub async fn submit_clip_password(
    ...
    if let Some(form) = &form.value {
        ...
        Ok(RawHtml(renderer.render(
            context,
            &["A password is required to view thsi clip"],
        )))
    }
}
